...
global ft_list_push_front         ; Make the function callable / visible from outside
...
; Allocate new item
    mov rdi, NEW_ITEM_SIZ         ; Load 16 bytes into malloc size parameter
    call malloc wrt ..plt         ; Allocate

    cmp rax, 0                    ; Test malloc return value
    jz .epilogue                  ; If null pointer, return directly

    mov [NEW_ITEM_PTR], rax       ; Else store the new pointer onto the stack
