...
TEST(ft_strcmp_null_string) {
    std::vector<std::function<int (const char *, const char *)>> f {
        ft_strcmp, strcmp
    };
...
TEST(ft_strcmp_equal_random_string) {
    ...
    for (int i= 0; i < res.n_tested; ++i) {
        try {
            ...
            if (ft_strcmp(s.c_str(), s.c_str()) != strcmp(s.c_str(), s.c_str())) {
                ++res.n_failures;
            }
...
TEST(ft_strcmp_different_random_string) {
    ...
    for (int i= 0; i < res.n_tested; ++i) {
        try {
            std::string s1 = generate_random_string(rng, 50000);
            std::string s2 = generate_random_string(rng, 50000);

            if (ft_strcmp(s1.c_str(), s2.c_str()) != strcmp(s1.c_str(), s2.c_str())) {
                ++res.n_failures;
            }
...
TEST(ft_strcmp_register_integrity) {
    char destination[12];
    return test_register_integrity(ft_strcmp, destination, "hello world");
}
